(() => {
    console.log('new volcengine message');
	const %s;
    let inputField = document.querySelector('textarea[style="height: 24px; max-height: 24px;"]');
	if(bFocus) {
		inputField.focus();
		inputField.click();
		const intext = `%s`;
	...
	if(bSent) {
		...
		//if (targetDiv) {
		  const targetNode = document.querySelector('div[class^="qaListContainer-"]');
